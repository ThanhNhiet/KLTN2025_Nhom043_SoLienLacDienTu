    ...
    useEffect(() => {
        ...
        useEffect(() => {
            ...
            useEffect(() => {
                ...
                const onReceivePin = ({ chat_id, pinnedMessage }: { chat_id: string; pinnedMessage: ItemMessage }) => {
                    if (!pinnedMessage) return;
                    if (chat_id && chat_id !== chatId) return;

                    setMessages((prev) => {
                        const exists = prev.some(msg => msg?._id === pinnedMessage._id);
                        if (exists) {
                            return prev.map(msg => (msg && msg._id === pinnedMessage._id) ? pinnedMessage : msg);
                        }
                    ...
                    setPinnedIDs((prev) => prev.includes(pinnedMessage._id) ? prev : [...prev, pinnedMessage._id]);
                ...
                socket.on('receive_pin_message', onReceivePin);
                socket.on('receive_unpin_message', onReceiveUnpin);
                socket.on ('render_message',onReceiveDelete);
                return () => {
                    socket.off('receive_message', onReceive);
                    socket.off('receive_pin_message', onReceivePin);
                    socket.off('receive_unpin_message', onReceiveUnpin);
                    socket.off('render_message', onReceiveDelete);
                };
            }, [socket, chatId]);
    ...
    const  handlePinMessage = async (message : ItemMessage,userID: string) => {
        ...
        const handleUnpinMessage = async (messageID: string) => {
            try {
                ...
                try {
                    socket.emit('unpin_message', { chat_id: chatId, unpinnedMessage_id: messageID });
                } catch (error) {
        ...
        const handleDeleteMessage = async (deletingMessage: ItemMessage | null) => {
            ...
            setMessages((prev) => {
                ...
                try {
                    socket.emit('del_message', { chat_id: chatId, message_id: messageID });
                } catch (error) {
